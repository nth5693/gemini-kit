...
// ═══════════════════════════════════════════════════════════════
import { initOrchestrator, teamStart, teamStatus, teamEnd, runWorkflow, smartRoute, getSessionHistory, } from './tools/orchestrator.js';
...
server.tool('kit_team_start', 'Start a new team session with a goal. AI will suggest best workflow.', {
...
Use \`kit_run_workflow\` to start the workflow or \`kit_team_status\` to check progress.`,
        ...
        return { content: [{ type: 'text', text: `Error starting session: ${error}` }] };
    }
});
// TOOL 19: Get Team Status
server.tool('kit_team_status', 'Get current team session status and progress', {}, async () => {
    try {
        ...
        return { content: [{ type: 'text', text: `Error getting status: ${error}` }] };
    }
});
// TOOL 20: End Team Session
server.tool('kit_team_end', 'End current team session and get summary', {
    ...
    try {
        ...
        return { content: [{ type: 'text', text: `Error ending session: ${error}` }] };
    }
});
// TOOL 21: Run Workflow
server.tool('kit_run_workflow', 'Execute a complete workflow (cook, quickfix, feature, refactor, review, tdd, docs)', {
...
Execute each step in order. Use \`kit_team_status\` to track progress.`,
        ...
        return { content: [{ type: 'text', text: `Error running workflow: ${error}` }] };
    }
});
// TOOL 22: Smart Route
server.tool('kit_smart_route', 'Analyze task and auto-select best workflow', {
...
Use \`kit_run_workflow\` with workflow="${result.workflow.name}" to start.`,
        ...
        return { content: [{ type: 'text', text: `Error in smart routing: ${error}` }] };
    }
});
// TOOL 23: List Workflows
server.tool('kit_list_workflows', 'List all available workflows', {}, async () => {
    try {
...
Use \`kit_run_workflow\` with the workflow name to execute.`,
        ...
        return { content: [{ type: 'text', text: `Error listing workflows: ${error}` }] };
    }
});
// TOOL 24: Session History
server.tool('kit_session_history', 'Get history of past team sessions', {
...
${sessions.map(s => `- **${s.name}** (${s.status})
  ...
  Agents: ${s.agents.length}`).join('\n\n')}`,
        ...
        return { content: [{ type: 'text', text: `Error getting history: ${error}` }] };
    }
});
// ═══════════════════════════════════════════════════════════════
// START SERVER
